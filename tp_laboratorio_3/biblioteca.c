...
int Validate_CharConNumeros(char ingresoTeclado[])
{
	int retorno;
	int i;
...
void Get_CharConNumeros(char mensaje[],char mensajeError[],char ingresoTeclado[])
{
	printf("%s",mensaje);
	fflush(stdin);
	scanf("%[^\n]",ingresoTeclado);

		while(Validate_CharConNumeros(ingresoTeclado)==0)
		{
			printf("%s",mensajeError);
			fflush(stdin);
			scanf("%[^\n]",ingresoTeclado);
